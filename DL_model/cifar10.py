...
# --- CONFIG ---
data_dir = "/mnt/c/zhaw/Ampli-FIRE/spectrograms_32"  # <-- Folder with spectrogram PNGs
labels_file = "/mnt/c/zhaw/Ampli-FIRE/labels.csv"    # <-- CSV mapping filenames to genres
batch_size = 32
num_epochs = 10
learning_rate = 0.001
img_size = 224
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model_path = "genre_classifier_dynamic.pth"
...
class SpectrogramDataset(Dataset):
    def __init__(self, folder, labels_csv, transform=None):
        self.df = pd.read_csv(labels_csv)
        self.folder = folder
        self.transform = transform

        # Map genres in CSV to fixed list
        ...
        self.labels = self.df['genre'].tolist()

        self.classes = fixed_genres_lower
        self.class_to_idx = {cls: idx for idx, cls in enumerate(self.classes)}
        self.idx_to_class = {idx: cls for cls, idx in self.class_to_idx.items()}
        print(f"✅ Using genres: {fixed_genres}")
...
# --- LOAD DATASET ---
dataset = SpectrogramDataset(data_dir, labels_file, transform=transform)
...
# --- PREDICT FUNCTION ---
def predict_genre(image_path, model_path="genre_classifier_dynamic.pth"):
